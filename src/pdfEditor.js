...
const windowsFontDirectory = path.join(process.env.WINDIR || "C:\\Windows", "Fonts");
const windowsFontCache = new Map();
...
export async function extractTextSpans(pdfBytes) {
	...
	for (let pageIndex = 1; pageIndex <= pdf.numPages; pageIndex += 1) {
		...
		const mergedSpans = mergeAdjacentSpans(rawSpans);
...
export async function applyTextReplacements(pdfBytes, spans, replacements) {
	...
	for (const replacement of replacements) {
		...
		const font = await resolveBestFont(pdfDoc, span, tuning, customFontCache, standardFontCache);
...
function chooseStandardFontKey(span) {
	const value = `${span.fontName || ""} ${span.fontFamily || ""} ${span.fontDescriptor || ""}`.toLowerCase();
	const isBold = value.includes("bold") || value.includes("black");
	const isItalic = value.includes("italic") || value.includes("oblique");
	const isMonospaceFamily = value.includes("monospace") || value.includes("monospaced") || value.includes("mono");
	const isCourier = value.includes("courier")
		|| value.includes("consolas")
		|| value.includes("lucidaconsole")
		|| value.includes("sourcecodepro")
		|| value.includes("robotomono")
		|| isMonospaceFamily
		|| looksMonospaced(span);
	const isTimes = value.includes("times")
		|| value.includes("georgia")
		|| value.includes("cambria")
		|| value.includes("garamond")
		|| value.includes("palatino")
		|| value.includes("bookantiqua")
...
async function resolveBestFont(pdfDoc, span, tuning, customFontCache, standardFontCache) {
	const windowsFontSpec = pickWindowsFontSpec(span, tuning);
	if (windowsFontSpec) {
		const cacheKey = windowsFontSpec.fileName;
		if (customFontCache.has(cacheKey)) {
			return customFontCache.get(cacheKey);
		}

		const fontBytes = await readWindowsFont(windowsFontSpec.fileName);
		if (fontBytes) {
			const embeddedFont = await pdfDoc.embedFont(fontBytes, { subset: true });
			customFontCache.set(cacheKey, embeddedFont);
			return embeddedFont;
		}
	}

	const standardFontKey = chooseStandardFontKey(span);
	return standardFontCache[standardFontKey] || standardFontCache.Helvetica;
}

function pickWindowsFontSpec(span, tuning = normalizeTuning()) {
	if (tuning.fontFamily !== "auto") {
		return { fileName: pickManualFontFace(tuning.fontFamily, tuning.fontStyle) };
	}

	const descriptor = normalizeFontDescriptor(span);
	const isBold = tuning.fontStyle === "auto"
		? descriptor.includes("bold") || descriptor.includes("black") || descriptor.includes("semibold")
		: tuning.fontStyle === "bold" || tuning.fontStyle === "boldItalic";
	const isItalic = tuning.fontStyle === "auto"
		? descriptor.includes("italic") || descriptor.includes("oblique")
		: tuning.fontStyle === "italic" || tuning.fontStyle === "boldItalic";

	if (descriptor.includes("calibri")) {
		return { fileName: selectFace(isBold, isItalic, "calibri.ttf", "calibrib.ttf", "calibrii.ttf", "calibriz.ttf") };
	}

	if (descriptor.includes("segoeui") || descriptor.includes("segoe ui")) {
		return { fileName: selectFace(isBold, isItalic, "segoeui.ttf", "segoeuib.ttf", "segoeuii.ttf", "segoeuiz.ttf") };
	}

	if (descriptor.includes("verdana")) {
		return { fileName: selectFace(isBold, isItalic, "verdana.ttf", "verdanab.ttf", "verdanai.ttf", "verdanaz.ttf") };
	}

	if (descriptor.includes("tahoma")) {
		return { fileName: selectFace(isBold, isItalic, "tahoma.ttf", "tahomabd.ttf", "tahoma.ttf", "tahomabd.ttf") };
	}

	if (descriptor.includes("cambria")) {
		return { fileName: selectFace(isBold, isItalic, "cambria.ttc", "cambriab.ttf", "cambriai.ttf", "cambriaz.ttf") };
	}

	if (descriptor.includes("times") || descriptor.includes("georgia") || descriptor.includes("serif")) {
		return { fileName: selectFace(isBold, isItalic, "times.ttf", "timesbd.ttf", "timesi.ttf", "timesbi.ttf") };
	}

	if (descriptor.includes("courier") || descriptor.includes("consolas") || descriptor.includes("lucida console") || looksMonospaced(span)) {
		return { fileName: selectFace(isBold, isItalic, "cour.ttf", "courbd.ttf", "couri.ttf", "courbi.ttf") };
	}

	if (descriptor.includes("arial") || descriptor.includes("helvetica") || descriptor.includes("sans") || descriptor.includes("swiss")) {
		return { fileName: selectFace(isBold, isItalic, "arial.ttf", "arialbd.ttf", "ariali.ttf", "arialbi.ttf") };
	}

	return { fileName: selectFace(isBold, isItalic, "arial.ttf", "arialbd.ttf", "ariali.ttf", "arialbi.ttf") };
}

function pickManualFontFace(fontFamily, fontStyle) {
	const isBold = fontStyle === "bold" || fontStyle === "boldItalic";
	const isItalic = fontStyle === "italic" || fontStyle === "boldItalic";

	switch (fontFamily) {
		case "Arial":
			return selectFace(isBold, isItalic, "arial.ttf", "arialbd.ttf", "ariali.ttf", "arialbi.ttf");
		case "Calibri":
			return selectFace(isBold, isItalic, "calibri.ttf", "calibrib.ttf", "calibrii.ttf", "calibriz.ttf");
		case "Segoe UI":
			return selectFace(isBold, isItalic, "segoeui.ttf", "segoeuib.ttf", "segoeuii.ttf", "segoeuiz.ttf");
		case "Verdana":
			return selectFace(isBold, isItalic, "verdana.ttf", "verdanab.ttf", "verdanai.ttf", "verdanaz.ttf");
		case "Tahoma":
			return selectFace(isBold, isItalic, "tahoma.ttf", "tahomabd.ttf", "tahoma.ttf", "tahomabd.ttf");
		case "Cambria":
			return selectFace(isBold, isItalic, "cambria.ttc", "cambriab.ttf", "cambriai.ttf", "cambriaz.ttf");
		case "Times New Roman":
			return selectFace(isBold, isItalic, "times.ttf", "timesbd.ttf", "timesi.ttf", "timesbi.ttf");
		case "Courier New":
			return selectFace(isBold, isItalic, "cour.ttf", "courbd.ttf", "couri.ttf", "courbi.ttf");
		default:
			return "arial.ttf";
	}
}

function selectFace(isBold, isItalic, regularFace, boldFace, italicFace, boldItalicFace) {
	if (isBold && isItalic) {
...
async function readWindowsFont(fileName) {
	if (windowsFontCache.has(fileName)) {
		return windowsFontCache.get(fileName);
	}

	try {
		const fontBytes = await fs.readFile(path.join(windowsFontDirectory, fileName));
		windowsFontCache.set(fileName, fontBytes);
		return fontBytes;
	} catch {
		windowsFontCache.set(fileName, null);
		return null;
	}
}

function normalizeFontDescriptor(span) {
	return `${span.fontName || ""} ${span.fontFamily || ""} ${span.fontDescriptor || ""}`
		.toLowerCase()
		.replace(/[,_-]+/g, " ")
		.replace(/\s+/g, " ")
		.trim();
